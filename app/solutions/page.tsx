  ...
  return (
    <>
      ...
      <main>
        <PageHero
          eyebrow="Solutions"
          title={<>Built for the work <span className="text-cyan-300">around your ERP.</span></>}
          description="These are practical starting points for extending your ERP—not off-the-shelf promises. Each solution is tailored to your process and platform."
        />
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {solutions.map((solution, index) => (
                <Reveal key={solution.title} delay={index * 80}>
                  <article className="group relative h-full overflow-hidden rounded-2xl border border-slate-200 p-7 transition-all duration-200 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-950/5 sm:p-8">
                    <span className="absolute right-7 top-6 text-5xl font-bold tracking-tighter text-slate-100">
                      0{index + 1}
                    </span>
                    <span className="grid size-12 place-items-center rounded-xl bg-slate-950 text-cyan-300">
                      <Icon name={solution.icon} className="size-6" />
                    </span>
                    <h2 className="mt-8 text-2xl font-bold tracking-tight text-slate-950">{solution.title}</h2>
                    <p className="mt-3 max-w-md text-sm leading-7 text-slate-600">{solution.description}</p>
                  </article>
                </Reveal>
              ))}
            </div>
